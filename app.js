...
const UI = {
  ...
  switchTab(tab) {
    ...
    if (tab==='discover') this._ensureDiscover();
  },
  ...
  renderAll() {
    this.renderSearch(); this.renderShows(); this.renderUpcoming(); this.renderLog(); this.updateStorageSize();
    // Invalidate discover cache when tracked shows change
  ...
  // Build a weighted genre profile from tracked shows
  _buildGenreProfile() {
    const shows = State.getShows();
    const freq = {};
    for (const show of shows) {
    ...
    // Sort genres by frequency, return top 6
  ...
  // Score a candidate show against the user's genre profile
  _scoreShow(show, genreProfile) {
    const userGenres = new Map(genreProfile.map(g => [g.genre, g.count]));
    const maxCount = genreProfile[0]?.count || 1;
    let score = 0;
    for (const g of (show.genres || [])) {
      if (userGenres.has(g)) score += (userGenres.get(g) / maxCount) * 3;
    }
    // Boost for still-airing shows
    if (show.status === 'Running') score += 1;
    // Boost for high rating
    if (show.rating?.average) score += show.rating.average / 10;
    // Slight recency bonus
    if (show.premiered) {
      const age = new Date().getFullYear() - parseInt(show.premiered);
      if (age < 5) score += 0.5;
    }
    return score;
  },

  async _fetchForYou(genreProfile) {
    const tracked = new Set(State.getShows().map(s => s.id));
    const queries = genreProfile.slice(0, 4).map(g => g.genre);
    ...
    await Promise.all(queries.map(async q => {
      try {
        const results = await TVMaze.search(q);
        for (const show of results) {
          if (!tracked.has(show.id) && !seen.has(show.id)) {
            seen.add(show.id);
    ...
    // Score and rank
    return candidates
      .map(show => ({ show, score: this._scoreShow(show, genreProfile) }))
  ...
  async _ensureDiscover() {
    const shows = State.getShows();
    const cacheKey = shows.map(s=>s.id).join(',');
    if (cacheKey === this._discoverCacheKey && this._discoverCache.length) return; // already loaded

    const forYouEl = document.getElementById('discover-foryou-list');
    if (!shows.length) {
      forYouEl.innerHTML = `<div class="empty"><div class="empty-icon">
        <svg width="36" height="36" viewBox="0 0 36 36" fill="none"><circle cx="18" cy="18" r="15" stroke="currentColor" stroke-width="2"/><line x1="11" y1="18" x2="25" y2="18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="18" y1="11" x2="18" y2="25" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
      </div><p>Track some shows first</p><p class="empty-sub">Add a few shows and we'll suggest more like them</p></div>`;
      return;
    }

    forYouEl.innerHTML = `<div class="discover-loading"><div class="spinner"></div><span>Finding shows for you…</span></div>`;

    const profile = this._buildGenreProfile();
    const results = await this._fetchForYou(profile);
    this._discoverCache = results;
    this._discoverCacheKey = cacheKey;
    this._renderDiscoverList('discover-foryou-list', results, profile);
  },
  ...
  async _doInterestSearch(q) {
    const spinner = document.getElementById('discover-spinner');
    const section = document.getElementById('discover-interest');
    const title = document.getElementById('discover-interest-title');
    const listEl = document.getElementById('discover-interest-list');

    spinner.classList.remove('hidden');
    section.classList.remove('hidden');
    title.textContent = `Results for "${q}"`;
    listEl.innerHTML = `<div class="discover-loading"><div class="spinner"></div><span>Searching…</span></div>`;

    const tracked = new Set(State.getShows().map(s => s.id));
    try {
      const results = (await TVMaze.search(q)).filter(s => !tracked.has(s.id)).slice(0, 12);
      const profile = this._buildGenreProfile();
      this._renderDiscoverList('discover-interest-list', results, profile);
    } catch(e) {
      listEl.innerHTML = `<div class="empty"><p>Search failed</p><p class="empty-sub">${e.message}</p></div>`;
    }
    spinner.classList.add('hidden');
  },

  _renderDiscoverList(elId, shows, genreProfile) {
    const el = document.getElementById(elId);
    const tracked = new Set(State.getShows().map(s => s.id));

    if (!shows.length) {
      el.innerHTML = `<div class="empty"><div class="empty-icon">
        <svg width="36" height="36" viewBox="0 0 36 36" fill="none"><circle cx="18" cy="18" r="14" stroke="currentColor" stroke-width="2"/><line x1="12" y1="18" x2="24" y2="18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
      </div><p>Nothing found</p><p class="empty-sub">Try a different search term</p></div>`;
      return;
    }

    const userGenres = new Set(genreProfile.map(g => g.genre));

    el.innerHTML = shows.map(show => {
      const img = show.image?.medium
        ? `<img src="${show.image.medium}" alt="" class="discover-thumb" loading="lazy">`
        : `<div class="discover-thumb no-img"><svg width="20" height="20" viewBox="0 0 20 20" fill="none"><rect x="2" y="3" width="16" height="10" rx="2" stroke="currentColor" stroke-width="1.6"/><line x1="6" y1="16" x2="8.5" y2="13" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/><line x1="14" y1="16" x2="11.5" y2="13" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/><line x1="5" y1="16" x2="15" y2="16" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg></div>`;

      const rating = show.rating?.average ? `<span class="discover-rating">★ ${show.rating.average.toFixed(1)}</span>` : '';
      const statusCls = show.status==='Running' ? 'badge-green' : 'badge-gray';
      const net = show.network?.name || show.webChannel?.name || '';

      // Highlight genres the user already likes
      const genres = (show.genres||[]).slice(0,3).map(g =>
        `<span class="badge ${userGenres.has(g)?'badge-green':'badge-mono'}">${g}</span>`
      ).join('');

      const year = show.premiered ? show.premiered.slice(0,4) : '';
      const isTracked = tracked.has(show.id);

      return `<div class="discover-card" data-show='${JSON.stringify(show).replace(/'/g,"&apos;")}'>
        ${img}
        <div class="discover-info">
          <div class="discover-name">${show.name}${year?` <span class="discover-year">(${year})</span>`:''}</div>
          <div class="discover-meta">
            <span class="badge ${statusCls}">${show.status==='Running'?'Airing':(show.status||'?')}</span>
            ${net?`<span class="badge badge-mono">${net}</span>`:''}
            ${rating}
          </div>
          <div class="discover-genres">${genres}</div>
          ${show.summary?`<div class="discover-summary">${show.summary.replace(/<[^>]*>/g,'').slice(0,100)}…</div>`:''}
        </div>
        <button class="discover-track-btn ${isTracked?'tracked':''}" data-show-id="${show.id}">
          ${isTracked?`<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><polyline points="1.5,6.5 5,10 11.5,2.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`:`<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><line x1="6.5" y1="1.5" x2="6.5" y2="11.5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="1.5" y1="6.5" x2="11.5" y2="6.5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>`}
        </button>
      </div>`;
    }).join('');

    el.querySelectorAll('.discover-track-btn:not(.tracked)').forEach(btn => {
      btn.addEventListener('click', async () => {
        const card = btn.closest('.discover-card');
        const show = JSON.parse(card.dataset.show.replace(/&apos;/g,"'"));
        btn.disabled = true;
        State.addShow(show);
        btn.className = 'discover-track-btn tracked';
        btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><polyline points="1.5,6.5 5,10 11.5,2.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
        toast(`Added ${show.name}`);
        this.renderShows();
        if (GAuth.isConnected()) {
          try {
            ...
            if (r.duplicates?.length) this.showDuplicatesModal(r.duplicates);
          } catch(e) { toast('Calendar sync failed: '+e.message,'error'); }
        }
      });
    });
